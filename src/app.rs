...
use components::title_area::TitleArea;

use crate::model::conversation::{Message, Conversation, Role::{Assistant,User}};
...
pub fn App() -> impl IntoView {
                ...
                match msg {
                    Ok(Txt(msg)) => {
                        set_conversation.update(move |c| {
                            c.messages.last_mut().unwrap().content.push_str(&msg);
                        });
                    }
                    _ => { break; }
                }
    ...
    view! {
        <Stylesheet id="leptos" href="/pkg/drover.css"/>
